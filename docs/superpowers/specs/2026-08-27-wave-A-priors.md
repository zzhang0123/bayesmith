...
在势能上加一个**常数**,NUTS 的轨迹**完全不变**:梯度相同,常数从每一个 Metropolis
比值里约掉。于是同一个 key 上跑两条链,一条带 factor 一条不带:

| 噪声声明 | 这个先验 | 实测 `max|路径差|` |
|---|---|---|
| radiometer | **恰好是平的**(+15.80169853) | `0.000e+00` / `0.000e+00`(两个 latent,300 抽样) |
| homoscedastic σ=1000 | `p(log A) ∝ A²`,不平 | **3.99** / **5.09** 个后验 sd |

**一对运行同时说出两件事**:factor 在采样器真正微分的那个势能里,而且它是那个平常数。
兄弟断言不可省——两条都忽略先验的链也会逐比特相同,所以第二行是第一行成立的条件。
