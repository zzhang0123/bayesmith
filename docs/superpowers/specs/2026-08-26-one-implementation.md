...
- **D39 — 归档的 manifest 与它的二进制**没有绑定**(做 D12 前置时发现)。**
  ...
  **第 3 步(仍开着):摘要绑定。** 它比第 1 步多抓的,正好是字节检查看不见的那一类
  ——**两份形状相同、内容不同**的归档(计数对得上,数值对不上),而那恰恰是最可能
  的真实误配:同一 campaign 两个晚上的归档结构相同。
  **安全的顺序是**:第 1 步(已做)→ 转换器 → 才是绑定。
  **先绑定而不备转换器,就是那条毁资料的路**,而这一层毁的是不可恢复的东西。
