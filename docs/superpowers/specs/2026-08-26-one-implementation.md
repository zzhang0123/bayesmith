...
7. **绑定契约不在本文重述**:模块契约 = 旧 spec §四 台账行 + 其
   docs/migration 页,开工先读。**两份都要,而且要按下面两问写下答案。**

   **【2026-08-29 补,起因见 D58——这条规则的现有措辞挡不住那次失败。】**
   开工前必须**写下**两问的答案:

   1. **这一行在 §四 的哪一栏?**(4.1 / 4.2 / **4.3 不迁移**)
      读 `docs/migration/<mod>.md` **不能回答这一问**——那是 cross-check 页
      不是授权页,读完它满足了本条前半句,栏位仍然不知道。
      而**「不迁移」的模块通常没有 migration 页**,所以「没有页」读起来像
      「还没写」,实际是「本来就不该有」。
   2. **点名这个模块的最新一条裁决是哪条?**
      `grep -rin '<mod>' docs/superpowers/specs/` **搜全部页**,取最新。
      **`-i` 不是可选的,而且只搜文件名不够**:实测 2026-08-29,一次自动核查
      找到了 `calibrate` 的 D11 却**漏掉 `npe` 的 D10**,因为 D10 的标题写作
      **「NPE 迁移」**(大写)。同时试:模块名、去 `.py` 的词干、大写/英文别名。
      **不要顺着读**:本仓的推翻是就地标注的
      (`【owner 已拍板】`、`【已触发并拍板】`、`【owner 已授权委托】`),
      而**被推翻的那一页永远不会告诉你它已经作废**。

   **顺手数一下这一栏还剩几条现行的——而且用眼睛数,不要只用正则。**
   实测 2026-08-29:§4.3 有**三条**,`calibrate`→D11、`npe`→D10 两条已被点名推翻
   (均 owner 2026-08-27 授权),**第三条仍然有效**:「整个流式证据层」八个模块
   (含 `reduced_basis.py`),它说的不是「不做」而是「**不移植,在这边通用化重写**」,
   归 §五 B11。

   **本条的第一版写的是「只有两条,整节作废」,那是错的**,而且是本会话第三次
   犯同一个病:自动核查找**加粗的模块名**,而证据层那一条加粗的是「**整个流式
   证据层**」这个中文短语,八个文件名在括号里用普通反引号写着。
   **匹配器只认一种写法,其余读成「不存在」,而「不存在」读起来像「查过了」。**
   前两次是私名普查的三个假阳性、和漏掉标题写作大写「NPE」的 D10;
   **这一次它进了一条已发布的断言**,而那条断言会让人去切证据层的模块。
   **先把整节读完,再写「一共几条」。**

   第 2 问是承重的那一问。实测:`calibrate` 的 §4.3 写着「不迁移」,而它已被
   推翻**两次**(2026-08-26 owner 裁「未迁移的全部迁移」;2026-08-27 **D11**
   点名本模块并授权迁移)。只读 §4.3 会得到一个反向的结论,而**据此回滚**
   比据此不做更糟——见 D58。
...
- **D33 — 一次发散的下降:返回还是拒绝(同上批次新增)。**
  ...
  **(i) 探针的判据和真实现的判据不是同一个,已对齐复核。** 探针挂的是
  「`found` 的某个 leaf 非有限」(**点**),而 `optimize.py:330` 的真判据是
  `~jnp.isfinite(objective(found))`(**该点处的目标值**)——注意它**挂在点上、
  按目标值判**,注释把这个区分写明了(「Attached to the POINT, because that is
  what a caller reads first」),与本条的裁定一致。两个判据**不等价**:点有限而
  目标溢出是可能的。所以用真判据重量了那唯一一条真依赖:该点 `g = nan`
  **且**该点的目标值 `= nan`,**两个判据都会拒**,改写结论不变。
  **一次用错判据量出来的分诊,和没量是一样的。**

  **(ii) rheplicant 侧不需要单独落这条。** bayesmith **已经实现了**
  (`optimize.py:326-341`),而 rheplicant 的 calibrator 会在 `calibrate`
  **接线那一批**继承它。所以这里没有「先在近端补一条 `error_if`」这个动作——
  那会变成两处实现同一条规则。
...
- **D39 — 归档的 manifest 与它的二进制**没有绑定**(做 D12 前置时发现)。**
  ...
  **【部分解除,2026-08-29,owner 点头后分步做;第 3 步仍开着。】**

  **第 1 步(已做):关掉静默的那一半,不动格式。** 这条缺陷不是「缺一个绑定」,
  是**一个不对称**——一个方向报错,另一个安静截断。`load_memory` 改为从打开的
  句柄反序列化,并比对**已消费字节 vs 文件大小**:manifest 少描述了内容,就被它
  没交代的那些字节抓住。实测正是那两份 fixture:**1976 字节里 144 字节从未被读**。
  **零格式改动、零版本提升、零 fixture 重写、已有归档照常可读。**
  它也不依赖 `template_projections` 恰好是最后一个叶子——**任何**位置的短缺都抓得到。

  **第 2 步:我先前的处方是错的,已按实测更正。** 我曾建议「让读取端能读旧版本」,
  作为任何格式提升的前提。**做不到**:版本之间差的是**字节布局**,不只是 manifest
  字段——本模块 docstring 自己记着「a version-1 file read by this code would
  deserialise the memory and then **run off the end of the file**」。
  把 `!=` 放宽成 `<=` 不是宽容,是**读垃圾**。
  **实际做的**:把拒绝写成**可操作的**,且两个方向给**不同**的建议
  (旧版本→回到写它的那个 rheplicant 重新归档;新版本→升级,别转换),
  并把这一层真正的约束写进消息里:

  > **这一层的前提是原始数据已经没了,所以一个它读不了的版本,就是一份没人救得回来
  > 的工作。因此:一次版本提升要么随附转换器,要么不发。**

  **第 3 步(仍开着)——而对抗式复核把它从「一件事」改成了「两件事」。**
  **【实测 2026-08-29,两条洞各由第二个 agent 独立复现,我又自己跑了一遍。】**

  **(a) 排列洞:一份归档,把它自己 manifest 的 `terms` 倒过来,二进制一字不动。**
  `save_memory` 按 archive 序写,`load_memory` 按 **manifest 序**建模板,
  equinox **按位置**填叶子——于是静态半边(来自 manifest)和动态半边(来自二进制)
  彼此错位。**两边都加载成功,字节检查看不见(同一个文件、同样大小)。**
  实测两 term 归档:约化 χ² 互换——**拟合坏的那一晚报 0.076、好的那一晚报 577**,
  而且各自顶着对方的标签。
  **「二进制的摘要」修不了它**:二进制逐字节没变,动的是 manifest。
  v3 的二进制里**没有任何逐 term 身份**,所以任何格式内的检查都看不见它。

  **(b) 崩溃窗口洞:`save_memory` 自己的 docstring 关于它的论证是错的。**
  它写着「crash mid-save leaves an orphan *binary*, which nothing reads,
  and the archive is simply absent rather than corrupt」——**那只在目标原本为空时成立**,
  而重新归档到一个固定路径是**常规操作**(重新定标之后)。进程死在两次写之间,
  **新二进制**就留在**旧 manifest** 旁边;若只有静态字段变了(改正的 `n_observed`、
  新的 `inputs` 摘要),**两份文件一样长**,字节检查看不见,加载成功——
  **数组是新的,每一个静态字段是旧的**。更糟的是过期的 `inputs` 会让
  `_reject_shared_inputs` **放行**一个它本该拒绝的 epoch。

  **所以第 3 步是两件事:**
  1. **manifest 里放二进制摘要** —— 关掉同形状误配 **以及** (b) 的崩溃窗口;
  2. **二进制里写入逐 term 身份** —— 关掉 (a) 的排列,而摘要对它无效。

  **安全顺序不变**:第 1 步(已做)→ 转换器 → 才是这两件。
  两条都已按 D39 的老办法**钉住而非修掉**(见守卫),`save_memory` 的 docstring
  已就地更正,而**我先前写的那段「剩下的是两份同形状归档,摘要能覆盖」两半都是错的**
  ——它会把下一位指向错误的修法,所以那段已改写。

  守卫:`TestTheManifestIsNotBOUNDToItsBinary` 已改写——静默那条改为断言拒绝
  (含字节数),另加匹配对照(反空洞)、以及两个方向的版本拒绝各一条。

  **两条实测撑着这次改动:**
  1. **零假阳性。** 忠实的 serialise/deserialise **从不留下字节**——八种形状实测
     (空元组、单标量、参差 shape、嵌套 dict、含 `None` 叶、bool+int、200×50 大数组、
     64 个小标量),`leftover` **全部为 0**。这是本次改动最重要的安全属性:
     **错误地拒绝一份好归档,毁掉的是不可恢复的东西。**
  2. **接缝变异 5 条 5 杀,其中一条是冲我自己的判断去的。**
     **M4 把版本检查从 `!=` 放宽成 `>`**——正是我先前推荐、随后自己否掉的那条改动。
     它被 `test_an_older_version_is_refused_with_what_to_do_about_it` 杀掉,
     **所以「宽容在这里是毁灭性的」这个判断现在有牙齿,不再只是一句 docstring。**
     其余四条:删掉字节检查、反转它、消息里去掉字节数、两个版本方向共用一句话。
     **一个靠论证得到的结论,在有东西能因它出错而变红之前,只值它最弱那一步的价钱。**
...
- **D44 — `compress_reduced_basis` 的归属波次(做 G6 时新增)。**
  ...
  **【前提被推翻,2026-08-29,见 D60。】** 本条把 `compress_reduced_basis` 排进
  Wave C 的理由是「与 G4 余下三个名字同批」——**而那三个名字实测是留守的**:
  `score_directions` 收 `ParameterSpace`+pipeline,`build_reduced_basis` 收
  pipeline/`State`/`NoiseModel` **且产出容器**,`basis_fidelity` 只读写两个 D12
  容器。按 D42/D31 的既有判据,三者都归 §〇 第 2 类留守,**不会有远端对应物**。
  于是本条的排期理由不再成立,**而结论要不要跟着变是一个新问题**:
  本条第 1 点(数值核心已在远端,实跑 1.76e-16)与第 2 点(其余每一件都读容器)
  **都还成立**,它们指向的其实是「`compress_reduced_basis` 本身也是留守」,
  而不是「它要等三个名字过去」。
  **【结清,2026-08-29 稍后:`compress_reduced_basis` **留守**——按 D42/D31 的既有
  判据,不是新判断。】** 先前这里写「留给下一位重谈」,而随后量到的一件事改变了
  它的紧迫性:**它不是收尾,它在 Wave D 的关键路径上。**

  > Wave D 先决 = D12 + G3 + **G6**;D12 已拍、G3 已落地;**G6 是 7 个里的 6 个**,
  > 而**第 7 个正是 `compress_reduced_basis`**。本条把它排进 Wave C「与 G4 余下
  > 三名同批」,而 **D60 实测那三名留守**——于是它**无批可同**,Wave C 走完了它
  > 也没被做。留着这条不决,等于把 Wave D 的门锁上。

  **实测签名(339 行)**:收 `basis`(`ReducedBasis`,D12 留守容器)与 `noise`
  (`NoiseModel`),读 `basis.rows`/`basis.c_ref`/`basis.support`,并触及 `State`
  与 `SqrtInfo`。**按 D42 判 `simulate_pairs`、D60 判那三个名字的同一把尺,它留守。**
  本条自己的第 2 点早就写着「它剩下的每一件都读容器」,只是当时把结论指向了
  「等三个名字过去」。

  **所以 G6 的第 7 个缺口不需要填——它根本不是远端的项目**,而 Wave D 的 G6
  先决**已满足**。
  **什么会推翻这条**:D12 被改判(容器过缝),或 `ReducedBasis` 本身迁移;
  两者都会让「读容器」不再是留守的理由,届时本条与 D60 一起重来。
...
- **D57 — Adam 的 beta 越界:远端静默给错答案(开 `calibrate` 波时发现)。**
  ...
  **为什么落在远端**:远端**自己的调用方**今天拿得到一个静默的错答案,而
  `minimize` 是公开 API。这条理由**独立成立**,与 rheplicant 无关。
  **【更正,同日晚些】** 初稿在这里还写了第二条理由——「`calibrate` 一旦切,
  近端 calibrator 成为门面,只在近端留就是把同一条规则写两遍」。**那条是错的,
  而且是在我量清楚之前写下的。** 后来量到:近端的 beta 守卫在
  `AdamCalibrator.__check_init__` 里,是**构造时**触发的,而远端 `minimize`
  **是个函数,没有构造这一步可以镜像**。所以近端那条**必须留守**,切换之后
  两边**各有一份**——但那不是「同一条规则写两遍」,是**同一条约束守在两个真正
  不同的入口上、以不同的时机触发**,和在 API 边界与数据库约束上各验一次同理。
  **裁定不变,被删掉的是一条我没验就写下的论据。**
...
- **D58 — 不是裁决,是一次流程失败的记录:我在两次部分阅读上各动手一次。**
  **【本条不需要 owner 追认。切换本来就被 D11 授权了——见下。留在登记簿里是因为
  失败形状值得记,不是因为有什么待决。】**
  ...
  **发生了什么,按顺序:**
  ...
  3. **我回滚了,并把回滚推上了 `main`**(`159cd5e`),同时写了一条「待 owner 追认」
     的登记。
  4. 然后才去读推翻链——**§4.3 早被推翻了两次**:
     * **2026-08-26**,owner 裁决「走一份实现路线:造适配层、**未迁移的全部迁移**」
       (§六 步骤 2 的引用框里);
     * **2026-08-27,D11 点名 `calibrate`** 并授权迁移,理由逐字是「**把 calibrate
       留在 rheplicant 会造出两个梯度 MAP 实现,直接违反本计划的一份实现法则**」。
  5. 重新落地(`0b66875`),并补上 D11 我先前漏掉的另一半(`2c18744`)。

  **第二个错比第一个重,而且更难看见。** 第一次是越权**做**;第二次是越权**撤销**
  ——撤掉的是 owner 已经授权的工作,而且 commit message 给的理由是错的。
  **两次是同一个毛病:在确立依据之前动手。** 区别只在于回滚**看起来**像负责任、
  像把控制权交还、像往保守一侧靠。**它是一次未经核实的 `main` 操作,打扮成了认错。**

  **发现冲突的那一刻,「保留」和「回滚」都不该选**——该做的是把推翻链读完。
  读的代价是几分钟;错误动作留在 `main` 上的代价是一条**错误的记录**,而错误的
  记录正是本程序反复在修的东西。

  **本仓的文档显式标注推翻关系**(「已拍板」「已触发并拍板」「已授权委托」),
  所以这一条是可以机械避免的。**开波清单已加两问,必须写下答案:**
  1. 这一行在 **§四 哪一栏**?
  2. **点名这个模块的最新一条裁决**是哪条?——用 `grep` **搜推翻**,而不是顺着读。

  第二问才是关键:它逼人去**搜索**,而顺着读一页永远不会告诉你它已经作废。

  **顺带,D11 的另一半我也曾漏掉。** 它写着「loss 方向守卫**随迁**,
  `test_loss_sense` **经包装重放**」,而我第一版把该守卫留在了近端,理由是远端的
  补救建议指向 `fit`/`nuts`(rheplicant 没有这两个名字)。**「经包装」三个字正是
  为这个问题准备的。** 已按 D11 实现:检测委托给 `check_loss_sense`,包装层翻译
  异常类并替换那句补救。该替换**按远端原文匹配**,上游一改措辞就会**静默失效**
  (消息悄悄退回 `fit`/`nuts`,而异常类、其余用例全都照样通过),所以两个方向都
...
- **D60 — `reduced_basis` 是**半切**,而 G4 并不欠债(开 Wave C 第三行时读出来的)。**
  **【本次委托下自定,2026-08-29:五个数组级名字委托,其余五个留守;计划未预见
  「G4 只做了 8 个里的 5 个算不算欠债」这一问,按「保守的一侧」规则自选。】**

  **先更正两个我自己先后给出的错答案**,因为它们都进过对话记录:
  (i)「§4.3 说不迁移,所以别切」——错,**§五 把 `reduced_basis` 和 `calibrate`、
  `npe` 写在同一行**(Wave C);(ii)「G4 只做了 5/8,所以这一行被卡住」——也错,
  **G4 页自己写着**「没做的三个…**三者都要先有容器那一侧的接线,所以它们跟
  Wave C 走**」,那是**同波工作**不是**前置**。

  **真正的形状,由 D42/D31 的既有判据读出来,不是新造的:**

  | 名字 | 判定 | 依据(实测签名) |
  |---|---|---|
  | `orthonormal_transform`、`orthonormalise`、`numerical_rank`、`select_svd`、`select_greedy` | **委托** | 只收数组;远端已有;**五个全部逐位相同**(x64 下 `max|Δ| = 0.0`,含 `orthonormal_transform` 的两个返回元素) |
  | `score_directions` | **留守** | 收 `space`+`pipeline`+`state_template` |
  | `build_reduced_basis` | **留守** | 收 pipeline/`State`/`NoiseModel`,**产出容器** |
  | `basis_fidelity` | **留守** | 只读写 D12 的两个留守容器 |
  | `ReducedBasis`、`FidelityReport` | **留守** | D12 |

  **这与 D42 对 `simulate_pairs` 的判法逐字同构**:四个公开件里只有一个碰
  `ParameterSpace`/pipeline/`NoiseModel`,那一个留守。这里是十个里有三个碰,
  加两个容器。**所以 G4 做的正是该做的那一半,不是做了一半。**

  **半切的先例已在:** Wave A 的 `numpyro_bridge` 与 `uncertainty`
  「各只切了一半,按契约留守其余,因此两者都**不在 `SWITCHED` 里**且 cross-check
  **保留**」。`reduced_basis` 同此处置。

  **两条新拒绝会随委托到达近端,按铁律 3 在此声明:**
  1. **D41 的 float32 拒绝**。远端每个入口都拒;近端不拒,因为它住在 x64 会话里。
     **实测安全**:`reduced_basis` 在 x64 之外的三个 config 测试文件里**只有散文
     引用**(引的是 `reduced_basis.py:171-180` 那个置换 bug),**没有一处调用**。
  2. 远端 `_rows` 的二维形状拒绝,近端的五个函数一条拒绝都没有。
  **近端十六条拒绝无一属于这五个**,所以**没有任何近端拒绝因委托而丢失**。

  **一条会因这次改动而静默腐烂的东西,已一并处理:**
  `reduced_basis.py:171-180` 被**八处**按行号引用(`config/sections/diagnostics.py`
  五处 + 三个测试文件),而**没有任何东西校验那个行号还指着对的代码**。
  该行范围在 `score_directions` 内(留守),但五个委托名里有两个
  (`orthonormal_transform` 57-105、`orthonormalise` 108-111)**排在它前面**,
...
- **D62 — D46 的复数拒绝**近端也要,而且是现在,不是等 `sqrtinfo` 委托**。**
  **【本次委托下自裁,2026-08-29;计划把它记成「随委托到达」,而那是错的。】**

  **先更正我自己在 Wave D 开波页写的一句**:那里写「D46 的复数拒绝**会随
  `sqrtinfo` 委托到达近端**——按铁律 3 要声明」,并把它排进次序第 7 步。
  **它永远不会到达。** `SqrtInfo` 是 **D12 的留守容器**——G6 登记页自己写着
  「容器按 D12 留守,所以返回的是 `SqrtInfo`、NamedTuple 与 dict」,也就是说
  **远端的数组级函数返回的是近端这个类**。近端 `SqrtInfo` 不会变成远端那个,
  所以那条拒绝不加就是没有。

  **而近端今天确实在静默给错答案**,D46 的最小复现原样重跑(x64):
  `R_1=[[1j]]`、`R_2=[[1]]`,`combine(t1,t2).fisher()` 返回 **`[[2.-0.j]]`**,
  而手算 `sum R^T R = (1j)^2 + 1^2` **恰为 0**。**绝对误差等于真值的全部,
  没有异常、没有警告。** 这与 B1 是同一类:本程序反复拒绝的那种静默错答案。

  **裁定**:在 `SqrtInfo.__check_init__` 加拒绝,**位置与理由与 D46 一致**
  (owner 2026-08-28 已对远端拍板),消息按近端词汇改写并指出出路。
  **附兄弟断言**:把拒绝所保护的那条算术单独钉住(双线性和恰为 0、酉形式为 2),
  因为拒绝落地之后那个行为就不可达了——这一手也是 D46 的先例。

  **两边各一份是对的,不是重复**:它们守的是**两个包各自的构造入口**,
  而 `SqrtInfo` 两侧各有一个类。与 D57 的 beta 守卫同理。

- **D63 — ~~`smooth` 的验收容差随链长增长~~ —— 本条**自我推翻**,2026-08-29。**
  **【自裁于 2026-08-29(Wave D 第 2 步开波);推翻的是我自己前一天的 D63。】**

  **原裁定错了两次,而两次都是同一个毛病:量了一个无界的量,又只用 3 个种子去拟合它。**

  原文写「实测 `rel ≈ 1.8 × eps × n^1.25`,验收写 `rtol = 10 * eps * n**1.25`」。
  加大种子数重测,**指数不复现,而且那条带子直接被击穿**:

  | 重测 | n=8 | n=32 | n=128 | n=512 | 拟合指数 |
  |---|---|---|---|---|---|
  | 3 种子/n(原) | 6.3e-15 | 1.0e-14 | 4.3e-13 | 3.9e-14 | **0.663** |
  | 8 种子/n | 2.1e-14 | 3.2e-14 | 4.3e-13 | 3.2e-12 | **0.862** |
  | 300/150/60 种子 | **5.7e-12** | 7.8e-13 | 1.0e-11 | — | — |

  在 300 个种子下,`10 * eps * n**1.25` 这条带子在 n=8 被**击穿 7 次**(最差
  5.68e-12 对带子 2.99e-14,**余量 0.01 倍**),n=32 击穿 2 次,n=128 击穿 1 次。
  「约为实测最差的 5 倍余量」这句话,是在 3 个种子的样本上说的。

  **成因**:被拟合的那个量是**逐元素相对误差**,而它的最差元素是一个**碰巧接近零的
  平滑均值分量**——n=8 时那个分量是 1.16e-02(数组尺度 6.08e-01),n=512 时是
  2.27e-05(尺度 2.36e+00)。分母越小比值越大,**这个量没有上界**,种子越多它越大。
  所以「它随 n 增长」是个假象:真正随 n 变的是「抽到一个更接近零的分量」的机会。

  **按尺度归一之后,分歧根本不随 n 增长**:`|a-b|_inf / max|a|` 实测
  6.8e-16、1.0e-15、7.4e-16、4.6e-16、5.4e-16(n = 8…1024),拟合指数 **-0.103**。
  绝对差同样是平的:1.55e-15、1.78e-15、1.78e-15。

  **新裁定**:验收用**绝对**容差,**与 n 无关**。现有测试的 `atol=1e-10` 已经把
  实测的每一个 case 覆盖了五个数量级。**不要**写 `n**1.25`。

  > **可推广的一条,而这条比容差本身值钱**:*一个比值的分母若可以任意接近零,
  > 那个比值就没有上界,而「拟合出来的指数」只是在拟合抽样运气。* 定容差之前
  > 先问**被除的那个数有没有下界**;没有下界就该用绝对容差,或者按数组尺度归一。
  > 我在前一天写下「一个界该随驱动它的那个量缩放」时是对的——**错在没有先确认
  > 那个量是可缩放的**。

  > **以及一条关于我自己的**:D63 原文的每个数字都是真的,推理读起来也顺。
  > 它错在**样本量**,而样本量是这份记录里唯一没被写出来的东西(「每个 n 取 3 个
  > 种子」被我写成了一句附注)。**一个由论证得到的结论,在有东西能因它出错而变红
  > 之前,只值它最弱那一步的价钱**——这句话是我上一个会话写的,而 D63 就是它的例子。

- **D64 — `smooth` **留守**,因为远端在刚性链上是**错的**;计划的「六个名字」实际是五个。**
  **【本次委托下自裁,2026-08-29。委托授权写明:**推荐与实测冲突时,实测赢**。
  计划 §四 G3 行与 G6 §2.3 都把 `smooth` 列进委托的六个名字里,本条不执行那一半。】**

  **判据不需要任何外部预言机。** 取 `phi = 1`、`process_std -> 0`:链**冻结**,
  每个 epoch 共用同一个 latent,所以平滑后验**必须收敛**。实测(x64,
  `chain_bank.stacked()`,theta = (0.4, -1.1)):

  | `process_std` | 近端 `mean[0]` | 远端 `mean[0]` |
  |---|---|---|
  ...
  **近端收敛并稳住五个数量级;远端偏离、翻倍,然后 NaN。** 有极限的那一侧是对的。
  同一扫描下 `process_std=1e-9` 处两侧均值相对差 **16.8**(1680 %)。

  **成因是结构性的,不是调参**:远端装配显式精度阵并调 `jnp.linalg.inv`,
  代价 `kappa(F)`;近端装配 QR 平方根并做两次三角回代,代价 `sqrt(kappa(F))`。
  **远端因此与它自己写下的设计相矛盾**——`marginal/chain.py` 的模块 docstring 说
  平方根形式正是「what keeps a thousand-epoch accumulation inside float64 where
  the explicit `(F, b)` form goes indefinite」。

  **`process_std=1e-9` 是被声明支持的输入**,不是我构造的极端:近端
  `LinearGaussianTransition` 的 docstring 自己写着「A chain that genuinely does not
  move is `process_std=1e-9`, not `0.0`」,且 `tests/evidence/test_transition.py:82`
  就构造了它。

  **两侧的测试都看不见它。** 近端每个 smoother fixture 用 `bank.PROCESS_STD`
  = 0.7071 或 `[0.4, 0.7]`;`tests/evidence/` 里最刚的链是
  `test_chain_conditioning.py:325` 的 6.32e-3,而它只断言 `np.isfinite()` 且走的是
  **filter** 而不是 smoother。远端 `tests/marginal/test_chain.py` 最刚的是 **0.02**
  (第 444 行)。**所以委托会通过全部现有测试并静默降级**——这正是本程序反复
  拒绝的那一类。

  **裁定三条**:
  1. `smooth` **留守**,近端保留平方根实现。
  2. 近端补一条**刚性链边界测试**(按 `boundary-validation.md`:扫
     `process_std × n_epochs × width`,断言方差为正且与稠密参照一致)。
  3. **向 bayesmith 报这个缺陷**,并在远端补同样的边界测试——因为今天
     **两侧都没有守卫**,上游即使想修也没有东西会变红。

  > **一条给下一位的**:这是本波第一次「委托」的正确答案是**不委托**。四件套的
  > (ii)「接缝变异红」在这里以另一种形式出现:*把远端换进来*就是那个变异,
  > 而它**没有让任何东西变红**——91 passed。**变异不红,说明缺守卫,不说明变异无害。**

  **§后续(同日,已修上游,但委托仍未落地——原因是发布门):**

  远端 `smooth` **已改用信息平方根**(bayesmith `_zeta_joint` + 两次三角回代),
  与 `chain_marginal` 同款。**修好之后两侧在每一个刚度上逐位相同**
  (`rel = 0.00e+00`,`process_std` 从 1e-1 到 1e-9)。
  三条 `xfail(strict=True)` **如设计地变红(XPASS)**,指名要删的 marker,
  ...
  **但近端 `smooth` 仍然留守,而理由变了,是铁律 5:**
  ...
  > **这正是 CLAUDE.md 记着的那个跨仓陷阱的镜像。** 记录在案的那次是
  > 「两条守卫之所以绿,只因为 editable 安装恰好停在一个未合并的分支上」;
  > 这一次方向相反——**一个正确的委托会因为同一个缝而变红**。两次的教训同一条:
...
- **D65 — D12 的**理由**是错的(equinox 的 `__check_init__` 从最派生类开始跑);
  结论仍**维持**,但改由别的事实支撑。这一条要 owner 过目。**
  **【本次委托下自裁**仅限于记录更正**;D12 是 owner 2026-08-27 的裁决,
  其**结论**本条不擅自推翻。】**

  D12 的表述是:「容器留守,因为 `__check_init__` 的异常身份在构造时**以基类优先**
  抛出,子类化无法翻译」。**后半句与 equinox 的实现相反。**

  实测(equinox 0.13.8,
  `.venv/lib/python3.12/site-packages/equinox/_module/_module.py:427-433`):
  ...
  `__mro__` 是**最派生类在前**,所以**子类的 `__check_init__` 先跑**,它里面的
  `raise` 会在基类那一次之前就中止整个循环。实测一个把
  `super().__check_init__()` 包在 `except StructureError -> raise
  StateValidationError` 里的子类:六个坏输入**全部**抛出 `StateValidationError`;
  合法输入照常构造;4 个 pytree 叶子,与近端相同。**所以子类化确实能翻译。**

  **但 D12 的结论在本模块仍然成立,理由换成实测的两条:**
  1. **`HyperTransition` 的委托是空的。** 远端该类共 30 行
     (`marginal/chain.py:205-234`),三个 `eqx.field(static=True)` 加一句
     `def at(self, values): return self.build(values)`。**它不含任何算术。**
     一个忠实的翻译子类必须重写 `__init__`(参数序)**和** `.at` 全文——两条
     `raise`,**以及远端没有的那一层 values 字典过滤**
     (`self.build({name: values[name] for name in self.hyper})`;实测近端 builder
     只看到 `['log_tau']`,远端 builder 看到 `['gain_slope','log_tau','t_rx']`)。
     **那是把远端 100 % 重新实现一遍**,换来零份算术的去重。
  2. **`LinearGaussianTransition` 的委托代价大于收益**:翻译子类要显式调
     `super().__check_init__()`,而 equinox 随后**还会沿 MRO 再调基类一次**——
     实测合法路径上基类检查跑了**两遍**(`['SUBCLASS','BASE','SUBCLASS-after-base','BASE']`)。
     此外近端正性消息 585 字符对远端 514,共享前缀 513,近端多出的
     `; and a quantity that is constant across the campaign is scope="global".`
     是 **rheplicant 的 config 词汇**,远端没有理由携带。

  **要 owner 定的**:D12 的**理由**既然错了,它在**别的模块**上的结论是否还站得住?
...
- **D70 — Wave D 第 4 步(`diagnostics` 消费面)。**这是本波第一次「远端没有数值缺陷」,
  ...
  * **先决已补**:`tests/evidence/test_held_out_oracle.py` 是**另一条推导**——
    ...
    **全程不做减法**),而出货的实现是「总量算一次、逐行减去一个 epoch」(`O(N)`)。
    所以减法路径上的符号错或陈旧总量**无处可藏**。它自带反空洞测试:
    把被留出的 epoch **留在里面**必须给出不同的数,否则这条预言机什么也没证明。
  * **拒绝一条都没委托**:`held_out_z` 先跑 `_campaign_arrays`(远端没有零行守卫);
    `coherent_mode` 把 `_refuse_mixed_templates` 留在调用之前。
  * **`epoch_residuals` 不委托**,因为那会**收紧一个已发布的表面**——
    很可能是改进,**但不是保守的一侧**。要 owner 认可。

  > **本格查出的最后一件,而它是本会话「守卫不可观察」的最干净标本**:
  > 删掉 `coherent_mode` 的 `_refuse_mixed_templates`,**六个诊断测试文件一条都不红**。
  > 原因良性——远端同一条拒绝的消息**只差开头一个大写字母**(`These` 对 `these`),
  > 而 `StructureError` 是 `ValueError`,所以每条已有的 `match=` 都落在后面的子串上。
  > 那条守卫**做着真事**(保住本包措辞,铁律 1 要求的)**同时与它自己的缺席无法区分**。
  > 已钉住那一个字符。**一条观察不到的守卫不是守卫。**

- **D71 — 第 7 步 `sqrtinfo` **已完成**,而 `marginalise` 留守的理由是:
  **委托它会搬走五条拒绝和零行算术**。**
  **【本次委托下自裁,2026-08-29。】**

  | 名字 | 判定 | 理由 |
  |---|---|---|
  | `SqrtInfo` | **留守** | D12 容器;带 D62 的复数拒绝;近端把曲率拼作 `fisher()`,远端 `information()` |
  | `marginalise_arrays` | **已委托** | 逐位相同(4 宽度 × 3 块大小 × 4 尺度 × 3 seed,最差 **0.0**),**两侧零拒绝** |
  | `marginalise` | **留守** | 见下 |

  **`marginalise` 的算术已经在远端了**——它自己的 docstring 就写着
  「The arithmetic itself lives in :func:`marginalise_arrays`」,而那一行本批已委托。
  近端 `marginalise` 剩下的是:**列的置换与命名簿记**、**五条拒绝**、
  以及构造近端 `SqrtInfo`。

  > **所以委托它会搬走五条拒绝、换来零行算术。** 这与 **D65** 的
  > `HyperTransition` 同形:*一个不含算术的名字,委托它买不到任何去重*。
  > 判据不该是「远端有没有同名的东西」,而是「**这次委托搬走的是算术还是拒绝**」。

  **顺带一条实测**:五条拒绝逐案与远端一致,**四条消息逐字节相同**,
  只有非有限 pivot 那条近端多出 `" Recompress that epoch."`(23 字符)。
  所以即便将来要委托,做法与 `calibrate` 的补救句替换同形,不需要新测量。

  **第 7 步的 D61 顾虑已经作废**:D61 说 `sqrtinfo` 要最后切,因为「本波每条验收都拿
  `SqrtInfo` 的字段表达,一旦它变成远端那个,比较两侧都是远端」。
  **`SqrtInfo` 留守,所以它永远不会变成远端那个**,那条顾虑不成立。
  D61 的**可推广部分**仍然成立并且今天又用到一次:*量具最后动*。
