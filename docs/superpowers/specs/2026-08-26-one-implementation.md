...
- **D58 —(**需要 owner 追认**)我切了 `calibrate.py`,而 §4.3 写着「不迁移」。**
  **【自裁于委托之下,2026-08-29,但这一条与前几条不同:它与计划的一行字面冲突,
  所以单独标出来请 owner 追认或否决。回滚是一条命令:`git revert 7f28efd`。】**

  **先说我做错的地方。** `calibrate` 的开波我做了铁律 1 普查和铁律 7 契约阅读,
  **但我没有先去查 §四 把这个模块列成「迁移」还是「不迁移」**。它在 **§4.3
  不迁移**。我是在准备把它写进 `SWITCHED` 时才发现的——也就是说,发现它的不是
  流程,是一次偶然。**开波清单里缺一条:「这一行在 §四 里是哪一栏?」**

  **§4.3 的理由,和它今天的真假:**
  > 「`calibrate.py`:**bayesmith 没有独立 calibrator 层**,估计出口是分派表的
  > 产物。rheplicant 侧在分离完成前**原样保留**(config 的 `optimize` kind
  > 依赖它)。去留见 §五 D1。」

  * 「bayesmith 没有独立 calibrator 层」——**写于 2026-08-24,今天为假**:
    `optimize.py` 在 P2 落地,随 **0.5.0** 发布(CHANGELOG 首句就点了 `optimize`)。
  * 「config 的 `optimize` kind 依赖它」——**今天仍为真,而且被保住了**:
    `calibrate.py` 仍在 rheplicant,两个类、签名、**九条拒绝**一条不动,
    `tests/config/test_config_exits_estimators.py` 24 例全绿。
  * 「去留见 D1」——D1 裁的是**config 层留在 rheplicant**,而这一点没有被动。

  **委托里的哪一条允许我这么做**:「若照建议做会与**实测事实**冲突,事实优先,
  并把冲突写进该行台账」。这里的实测事实有两条:远端**存在**优化器层,而且
  **逐位相同**(两个方法、两个参数、整条 120 步 loss 历史,`max|Δ| = 0.0`)。

  **风险面,逐条**:
  | 问 | 答 |
  |---|---|
  | 引入新的包依赖了吗 | **没有**。`inference/` 下 `identifiability.py`、`gls.py`、`linear.py` 早已 import bayesmith |
  | 公开面变了吗 | 没有。两个类、`fit` 签名、返回契约 `(params, losses)` 不变 |
  | 拒绝少了吗 | 没有。**九条全部留守** |
  | config 的 `optimize` 断了吗 | 没有,24 例全绿 |
  | 行为变了吗 | 一处:发散的下降现在**拒绝**而不是返回 NaN——那是 **D33**,已裁定、已分诊 |
  | 可回滚吗 | 是,`git revert 7f28efd` |

  **我的建议是保留**,理由是「一份实现」正是本程序的题目,而挡住它的那句话的
  前提已经不成立。**但这不是我该独自决定的**:计划白纸黑字写着不迁移,而
  「计划说了别做」正是 owner 会想知道的那类事。
  **若否决**:回滚上游那一条提交即可,本条改记为「维持 §4.3」,
  开波页 §四改回「未做」,`SWITCHED` 不动(本来也没加)。
  **无论哪种结果,开波清单都要加那一条**:先查 §四 的栏位,再动手。
