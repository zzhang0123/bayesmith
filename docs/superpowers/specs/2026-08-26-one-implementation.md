...
- **D63 — `smooth` 的验收容差随链长增长,不是一个常数。**
  **【本次委托下自裁,2026-08-29;计划只写了「待定容差」。】**
  近远两侧 `smooth` 签名逐字符相同,但**不是逐位**。实测(x64,每个 n 取 3 个种子的最差):

  | n_epochs | 8 | 32 | 128 | 512 |
  |---|---|---|---|---|
  | worst rel | 5.6e-15 | 2.5e-14 | 4.0e-13 | 8.4e-13 |

  **拟合:`rel ≈ 1.8 × eps × n^1.25`**(eps = 2.22e-16)。
  外推到 B11 实测的千 epoch 归档规模(**12,007 leaves**)是 **4.7e-11**。

  **成因是累积舍入而不是另一个算法**:RTS 平滑是顺序递推,每个 epoch 喂下一个,
  所以结合序的差异沿链复利。指数 1.25 与此一致;一个真的算法差异不会这样缩放。

  **裁定**:验收断言写成 **`rtol = 10 * eps * n_epochs**1.25`**(约为实测最差的 5 倍余量),
  **不写常数**。理由与本仓 `machine_factor()` 的用法同形:
  **一个界该随驱动它的那个量缩放**,否则它在小规模上过松、在大规模上误报。
  **并且断言要连 n 一起扫**——只在一个 n 上比,看不出它在增长,也就看不出
  「常数容差」为什么是错的形状。
