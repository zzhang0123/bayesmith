...
| `chain` **实际落地** | **切了一个** | `chain_marginal`(递推本体)。`smooth` 因 **D64** 留守;两个 transition 类因 **D65** 留守;`chain_log_likelihood` 是 `chain_marginal` 的一行包装,随之委托而无需自身改动 |
...
* `smooth` —— ~~容差已定,见 D63~~ **这一整条作废,两次,见 D63(自我推翻)与 D64。**
  1. **容差的形状错了**:`n^1.25` 不复现(加大种子后拟合指数 0.663 / 0.862),
     且 `10*eps*n**1.25` 这条带子在 300 个种子下于 n=8 被击穿 7 次。成因是被拟合的
     量(逐元素相对误差)**没有上界**——最差元素是一个碰巧接近零的均值分量。
     按尺度归一后分歧**根本不随 n 增长**(指数 -0.103)。用绝对容差。
  2. **而且根本不该委托**:远端 `smooth` 在刚性链上是**错的**。`phi=1`、
     `process_std` 下降时链冻结,答案必须收敛;近端稳在 0.454968749 五个数量级,
     远端走到 **0.931437**(恰好两倍)然后 **nan**。远端装配显式精度阵求逆,
     付 `kappa(F)`;近端 QR 平方根,付 `sqrt(kappa(F))`。**`smooth` 留守。**

  **这一格是本页最贵的一格**:它把「两侧不逐位」当成了容差问题,于是去测容差,
  而没有问「**哪一侧是对的**」。答案不需要预言机,只需要一个必须存在的极限。
