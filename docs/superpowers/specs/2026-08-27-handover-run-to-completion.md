...
**`calibrate` 已切(e-RHINO `7f28efd`,四件套齐备)——⚠️ 但它在 §4.3 的「不迁移」栏里。**
**这是一次与计划字面冲突的改动,登记为 D58,等 owner 追认或否决。**
我做开波时**漏了一步:没先查 §四 的栏位**,是准备写 `SWITCHED` 时才发现的——
**开波清单要加这一条**。§4.3 的理由「bayesmith 没有独立 calibrator 层」写于
2026-08-24,**今天为假**(`optimize.py` 随 0.5.0 发布)。九条拒绝全留、签名不动、
config 的 `optimize` 24 例全绿、逐位相同、可 `git revert 7f28efd`。
**我的建议是保留,但这不是我该独自决定的。**
 开波页:
`2026-08-29-wave-C-calibrate-opening.md`。它是最可切的一行:
