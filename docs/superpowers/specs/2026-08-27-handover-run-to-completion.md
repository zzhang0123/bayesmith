...
- **实测于 2026-08-29 晚(Wave D 第 2 步收尾)**:README 计数 **10853**
  (守卫按等号钉,数字取自它自己的失败消息);**串行全套 10194 passed、
  637 skipped、exit 0**(24 min 10 s,即 coverage job 那一次);
  **e2e 第二阶段 21 passed、exit 0**;coverage **本地 89.57 %**,README 写
  **截断**后的 **89.5**。
...
2. **`epoch_residuals` 的委托** —— 需要 owner 认可「收紧一个已发布表面」。
   适配器与两条会新增的拒绝都写在 D70。
3. ~~两条剩余的「没有守卫」~~ **已补**(`test_diagnostics_unguarded.py`,变异 2/2)。
   两条都**不可能被现有 fixture 触及**,而那是原因不是疏漏:`tests/evidence` 里
   每个先验都是高斯的——处处有限、处处为正。非有限曲率那条要一个在求导点奇异的先验,
   非正定那条要一个**减少**信息的先验。先验在本包是只读 `log_prob` 的鸭子类型,
   所以两个各三行(`sum(log x)` 与 `+K/2 sum(x^2)`)。
