...
- README 计数 **10794**;**全套 10193 passed + 21 (e2e)**,覆盖率 **89.34 %**。
- **两仓 tip**:e-RHINO `b2ae646`、bayesmith `969efae`。
  **用 `git ls-remote` 数,不要读本地 `origin/main`。**
  推送顺序:**e-RHINO 先,bayesmith 后**(bayesmith 的 Seam job checkout 的是
  `e-RHINO@main`)。**注意本会话反复踩到的一件**:连续推送会把上一次的 CI run
  取消掉——`rheplicant` 的 Coverage job 因此有三次没跑完。推完等它。

**最近一次会话(2026-08-29)落地:Wave C 三行 + Wave D 的第 0、1 步。**
逐条见 §四/§五。**下一件是 Wave D 次序第 2 步:`chain` 的六个 G3 名字**
——它只要 `sqrtinfo` **存在**、不要它切换,与第 3 步互相独立。
验收形状已由 **D63** 定死:`rtol = 10 * eps * n**1.25`,**扫 n**,不写常数。
...
### 三之五、本会话踩了同一个病**五次**,值得下一位先读

**形状每次相同**:一个匹配器只认一种写法,其余读成「不存在」,而**「不存在」读起来
像「查过了」**。五次里有三次进了已发布的断言。
...
shrinkage 名字委托(e-RHINO `b2ae646`,逐位,变异 4/4)。
**下一件是第 2 步 `chain` 的六个 G3 名字。**
...
* **G3** 已落地 2026-08-27,`evidence/chain.py`,六个名字全数。
