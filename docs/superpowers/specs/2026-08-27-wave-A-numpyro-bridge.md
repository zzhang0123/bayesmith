...
> **更正(同日,由 CI 给出)**:上面这一段的结论**在另一台机器上不成立**。本机的
> 1.4e-6 在 CI runner 上是 **1.89e-2**——六个数量级的余量实际只有不到两个,而且方向
> 不可预测,因为 leapfrog 轨迹是混沌的。**重钉一个更小的数是同一个错误加一个容差**。
> 那条断言已被删除,平先验那一半改由**势能**上的确定性测试承担。
> 见 `2026-08-27-ci-flat-chain.md`。
