...
| **B5** | `1/σ²` 权重公式有两个家：`inference/linear.py:1421` 与 `inference/noise.py:339-349`，在 NaN σ 上分叉（共轭路径传播 NaN，`inverse_variance` 映射为 0）。文档称之为刻意，但它是「derive, don't re-spell」唯一一处刻意的破例。 | bayesmith 收敛为**一个** helper，带显式 `mask_nan` 形参。 | `[互证]` |
...
| **B7** | Gibbs 诊断只有联合 χ² 单标量的 split-r̂，无逐参数 r̂/ESS。 | bayesmith 的 Gibbs 产物应带逐参数 r̂/ESS（接 `numpyro.diagnostics`）。**注意**：rheplicant 的 1.05 阈值论证是诚实的（单链单标量下 1.01 是噪声主导），逐参数版需要**自己的**阈值论证，不能把这个照抄过去。 | `[R2]` |
...
**增量 2（未做）**：接进 12 个既有消费点。`normal_operator` 只用 `weight[name]*pushed**2`，即只需二次型，所以那里是局部改动；`fisher_information`、`gaussian`、证据层白化行同理。
