...
> **⚠️【本节整体已作废,2026-08-29 核实。】** 本节名下**只有两条**,而**两条
> 都被点名推翻了**:`calibrate.py` 由 **D11**、`npe.py` 由 **D10**,均为
> **owner 2026-08-27 授权**,上位是 2026-08-26 的「未迁移的全部迁移」。
> **所以这一节没有一条现行条目。** 保留原文不删,是因为它曾被当作现行依据
> 使用过一次,并据此回滚掉了已授权的工作(见 **D58**);读到它的人需要看到
> 它为什么不再适用,而不是发现它不见了。
>
> **给下一位的两条,是做这次核实时踩出来的:**
> 1. **搜推翻要不分大小写,而且要搜概念不只搜文件名。** 一个自动核查在这里
>    找到了 `calibrate` 的 D11、**漏掉了 `npe` 的 D10**——因为 D10 的标题写作
>    **「NPE 迁移」**(大写),而查询串是 `npe.py`。`grep -rin`,并同时试
>    模块名、去掉 `.py` 的词干、以及它的大写/英文别名。
> 2. **一节可以整体过期。** 逐个模块查会得到「这一条恰好被推翻了」的印象;
>    整节查才看得见「这一节没有一条还活着」。查栏位时顺手数一下这一栏还剩
>    几条现行的。
