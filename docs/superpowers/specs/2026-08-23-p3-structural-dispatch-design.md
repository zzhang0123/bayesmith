...
> **实测更正（写 P3b spec 时，2026-08-23）：这一段有两个独立的错误。**
>
> **(1) `C` 的两个符号都反了。** `log q(y) = −½rᵀMr + ½ log det M − (n/2) log 2π`，
> 所以 `log w = log p − log q` 里二次项与 log-det 项必须**反号**：
> `C = −½ log det M + (n/2) log 2π`。实测（4 维，对 scipy 校验）：本文的 `C`
> 误差 +2.396e-01，更正后逐位为零。
>
> **(2) `C` 在 MH 比值中并不抵消。** 按 §5.4 在 `x'` 处重建提议，则 `M' ≠ M`，
> 残留 `½(log det M' − log det M)`——恰在 σ̂ ≠ σ̂′ 时非零，即路径 (B) 存在的
> 全部情形。实测该链偏 27 个标准误（两个种子复现），平稳分布的闭式是
> `π ∝ p(x)·det M(x)^{−1/2}`，**而接受率看起来比正确的链还健康**。
> 修法见 `docs/superpowers/specs/2026-08-23-p3b-dispatch-execution-design.md` §5.1 与 §5.3。
...
> **实测更正（2026-08-23）**：**「在 `x'` 处再建一次」是这里唯一需要改的字。**
> 状态依赖的 σ̂ 使 `M' ≠ M`，于是 `log det` 不抵消——而 `log det` 正是无矩阵
> 方法算不动的那一个量。**把 σ̂ 改成只依赖块外状态 `z` 的确定性函数**，提议
> 就是真正的独立提议，`M' = M`，`log det` 真正抵消，且成本从 3 次 CG 降到 2 次。
> 实测三条链对 1-D 求积真值：状态依赖无 log det 偏 +0.0357；带 log det
> −0.0014；独立提议 **−0.00046**。
> **本节的结论（反向密度必需）不变，理由与机制改变。** 见 `docs/superpowers/specs/2026-08-23-p3b-dispatch-execution-design.md` §5.3。
