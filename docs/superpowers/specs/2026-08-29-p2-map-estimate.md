...
优化层面的失败返回 `Refused(reason)`，无 latent 的图返回
...
求解复用 `sensitivity._newton` 的阻尼 Newton 算术，但 P2 自己解释结果：

1. 环境不是 float64：`Refused`，出路是把**构图也放进** `jax.enable_x64(True)`；
2. 目标、梯度或 Hessian 非有限：`Refused`，出路是改起点、尺度/坐标，或用 NUTS；
3. Newton 未收敛：`Refused`，绝不返回最后一次“看起来合理”的 iterate；
4. 驻点的负 log joint Hessian 非正定：`Refused`，因为它是 saddle/flat ridge，不是 MAP；
5. 无 latent：`NotApplicable`，而不是空字典冒充估计。
...
1. 线性高斯图使用非单位先验宽度；MAP 与 `wiener_solve` 对拍，且成功值直接传给
   `local_block(..., priors=True)`。
2. 一维 Neal funnel 接受并落在解析众数 `(y,x)=(-4.5,0)`；这直接杀掉借用
   `_refuse_entangled_selection` 的实现。
3. `child ~ Normal(parent, .5)` 的层级似然图接受。
4. 两个 ImproperUniform latent 只以和进入数据，完整 Hessian 有真零方向：拒绝文本点明
   proper prior/删冗余参数等出路。
5. ImproperUniform logistic likelihood 的上确界只在 `x→∞` 取得，没有有限 MAP；迭代
   失败返回 `Refused`，不泄漏最后 iterate。
6. float32 按名拒绝；无 latent 返回 `NotApplicable`。
...
| **D77** | 沿用哪套拒绝？ | 一条也不沿用 prior-sensitivity 的三条模型分类；P2 只拒绝精度、非有限、未收敛和非极小这些 MAP 自身失败。 |
