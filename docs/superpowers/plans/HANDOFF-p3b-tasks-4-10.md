...
### 1. 暂存与恢复都按**显式路径**，不按目录、不按 HEAD

**这条在本轮犯了两次，第二次是在刚写完第一次的警告之后。** 两次的根因相同：
**用了一个作用域比意图宽的命令。**

| 事故 | 用的命令 | 意图的作用域 | 命令的实际作用域 | 后果 |
|---|---|---|---|---|
| 变异恢复 | 从 HEAD 复制回去 | 「撤销我刚才的变异」 | 「回到最后一次提交」 | 抹掉了此后产生的**未提交**改动，一次工作作废 |
| 提交文档 | `git add docs/` | 「暂存我改的那两个文件」 | 「这个目录下的一切」 | 把 owner 未跟踪的 313 行 spec 卷进了一条讲别的事的提交 |

> **规矩，两条都是同一条**：
>
> - **恢复**：备份取自**开始那一刻文件的磁盘内容**（自己的 scratch 目录），恢复只从那份备份读，恢复后逐字节校验（sha256）。**永远不要用 `git checkout` / `git stash` / `git restore` / 从 HEAD 复制来「恢复」**——git 只知道最后一次提交，而变异测试的使用场景**恰恰是有未提交改动的时候**，两者定义上冲突。
> - **暂存**：`git add <显式路径列表>`。**不要 `git add -A`、`git add .`、`git add <目录>/`**，除非你刚刚看过 `git status` 且确认那个作用域里每一个文件都是你要提交的。有 subagent 在跑、或有别人的未跟踪文件在树里时，这一条尤其承重。
>
> 判据：**在敲下命令之前，说出它会碰哪些文件。说不全就换一个更窄的命令。**

（本轮那次误提交已用 `git reset --soft HEAD~1` + `git restore --staged <path>` 拆开；
分支未推送，改写 tip 是安全的。若已推送则不要改写，另开一条提交说明即可。）
